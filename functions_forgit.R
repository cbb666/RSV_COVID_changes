...
genRateGlobal.fromProp <- function(df, n.level, returnMC = FALSE) {
  df <- df[df$Group != "Global",]
  df <- df[!(is.na(df$est.x)|is.na(df$est.y)),] 
  if(nrow(df)==0){
    return(NULL)
  }
  # df <- left_join(df, pop_region.raw)
  ...
  df.MC$N_prop <- df.MC$Pop * df.MC$rate * df.MC$prop  #人口数 * 住院率 * 严重下感比例 = 严重下感住院数/院内死亡数 1000个
  df1 <- df.MC %>% group_by(AGEGR, Group,Year,n.all) %>%  # Impute.x,Impute.y,, n.impute # age是唯一值，group有两个，对应n.all也有两个 1*2
    dplyr::summarise(N.est = round(median(N_prop),3),
                     N.lci = round(quantile(N_prop, 0.025),3),
                     N.uci = round(quantile(N_prop, 0.975),3), #地区严重下感住院数
                     Pop = Pop[1],
                     prop.est = median(N_prop/N)*100, 
                     prop.lci = quantile(N_prop/N, 0.025)*100,
                     prop.uci = quantile(N_prop/N, 0.975)*100  # 地区严重下感住院数， 占总下感比例
    )
  df1$IR.est <- with(df1, N.est/Pop*1000)
  df1$IR.lci <- with(df1, N.lci/Pop*1000)
  df1$IR.uci <- with(df1, N.uci/Pop*1000)
  
  if(nrow(df1) == n.level){  # n.level = 2
    ...
    df2 <- df2 %>% group_by(AGEGR, Group,Year,n.all) %>%  
      dplyr::summarise(N.est = round(median(N_prop),3),
                       N.lci = round(quantile(N_prop, 0.025),3),
...
res <- ggsave(
  res,
  filename = paste("Figures1/",outcome,"_prop_OR.pdf", sep = ""), width = 8, height = 6 ) # Each$Index1[1],"/",Each$Index[1], 
    ...
    res <- ggplot(data = dataset,
      ...
      scale_x_discrete(name = NULL, # Association of proportion of requiring MV/ICU with 
                       labels = c("2019vs2020"="2020", "2019vs2021"="2021", "2019vs2022.3"="Apr 2021 to \nMar 2022")) +
      #          "GAGR3237"="Gestational week: 32-<37", 
      #          "chdYES"="Congenital heart disease")) +
      facet_wrap(~AGEGR, ncol = 2 ) +    # , scales = 'free_y'
